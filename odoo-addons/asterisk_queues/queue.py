...
GRANT
barrier=# grant all on asterisk_queue_member to asterisk;
GRANT
barrier=# grant all on asterisk_queue_rule to asterisk;
GRANT
"""
...
class Queue(models.Model):
    ...
    operators = fields.Many2many(comodel_name='barrier.operator')
    calls = fields.One2many('barrier.queue_call', inverse_name='queue')




class QueueCall(models.Model):
    _name = 'barrier.queue_call'
    _description = 'Queue Call'
    _rec_name = 'caller_number'

    queue = fields.Many2one('barrier.queue', required=True)
    resident = fields.Many2one('barrier.resident')
    caller_number = fields.Char(required=True)
    state = fields.Selection(QUEUE_CALL_STATES)
    states = fields.One2many(comodel_name='barrier.queue_call_log',
                             inverse_name='call')
...
class QueueCallLog(models.Model):
    _name = 'barrier.queue_call_log'
    _description = 'Queue Call Log'

    call = fields.Many2one('barrier.queue_call')
...
class Operator(models.Model):
    _name = 'barrier.operator'
    _description = 'Operator'

    name = fields.Char(required=True, index=True)
    exten = fields.Char(required=True)
    queues = fields.Many2many(comodel_name='barrier.queue')
    state = fields.Selection(OPERATOR_STATES, default='disconnected')
    states = fields.One2many(comodel_name='barrier.operator_log',
...
class OperatorLog(models.Model):
    _name = 'barrier.operator_log'
    _description = 'Operator Log'

    operator = fields.Many2one('barrier.operator')
